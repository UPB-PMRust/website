...
| [defmt](https://crates.io/crates/defmt) & [defmt-rtt](defmt-rtt)| Ultra-fast and efficient space logging framework for microcontrollers. | Sends diagnostic text messages and telemetry data from the board directly to the computer console via the Segger RTT protocol without consuming unnecessary Flash memory. |
