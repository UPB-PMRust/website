...
From the software poing of view, the logic is now reversed: we wait for rising edge, not for the falling one. With the new logic applied, due to motor vibration paper pendulum creates insane amount of false-triggers, so my next goal is to reconsider the pendulum material and position in order to minimize the vibration-triggers. Increasing the debouncing interval may also resolve the issue.
