...
:::info
The Raspberry Pi Pico 2 uses the same target triple as the STM32U545 (`thumbv8m.main-none-eabihf`). This means you can build for the Pico 2 using the exact same command shown above.
:::
...
When working in Rust for the STM32 Nucleo-U545RE-Q, tools such as `rust-objdumb` can be used to inspect the compiled binary and analyze its memory layout. The output includes various sections that are crucial for embedded development.
