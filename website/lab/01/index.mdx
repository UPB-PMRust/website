...
This lab will teach you how to debug and run programs written in Rust and [embassy](https://embassy.dev) on the Raspberry Pi Pico and NUCLEO-STM32 boards.
...
- How to use `probe-rs` for RP2/STM32 processors
...
Each development board defines its own way of uploading software to it. Some boards, like the Raspberry Pi Picos or STM32s, provide a *simulated* USB drive
...
<Tabs>
  <TabItem value="stm32u5" label="NUCLEO-STM32U545" default>
...
<Tabs>
  <TabItem value="stm32u5" label="ST-LINK/V3SET" default>
...
The ST-LINK firmware is maintained by ST and is regularly updated through STM32CubeProgrammer. It is proprietary but widely supported across ST’s software ecosystem, including STM32CubeIDE, OpenOCD, and many others third-party options.
...
For wiring and pinout information, refer to the ST-LINK/V3SET’s [user manual](https://www.st.com/resource/en/user_manual/um1075-stlinkv2-incircuit-debuggerprogrammer-for-stm8-and-stm32-stmicroelectronics.pdf) which includes details on connecting to SWD/JTAG interfaces.
...
communicate with and control the target device (the Raspberry Pi Pico 2/STM32U545 in this case). These software tools provide 
...
when developing for embedded systems like the Raspberry Pi Pico 2 or STM32U545 because the 
...
:::info
For the STM32U545 will be the same, since we have the same target.
:::
...
<Tabs>
  <TabItem value="stm32u5" label="NUCLEO-STM32U545" default>
To be able to properly run the code, the STM32U545 microcontrollers expect the binary to follow a specific format defined by their memory map and startup sequence.
The compiler and the linker, both provided by Rust, have to make sure that the generated binary code places some regions correctly, otherwise the STM32U545 will not start.
  </TabItem>
...
Finally, the firmware image concludes with an `end marker` section, which indicates the end of the binary. This ensures that no additional data is written beyond the intended memory space, protecting critical regions of flash and RAM from accidental overwrites during updates or execution.
...
<Tabs>
  <TabItem value="stm32u5" label="ST-LINK/V3SET" default>
When working in Rust for the STM32U545, tools such as `rust-objdumb` can be used to inspect the compiled binary and analyze its memory layout. The output includes various sections that are crucial for embedded development.
...
To run firmware on the Raspberry Pi Pico 2 or STM32U545, the 
...
When developing Rust firmware for the Raspberry Pi Pico 2 or STM32U545, `probe-rs` is the 
...
Pi Pico 2/STM32U545. Instead of just compiling and running a program on the host computer, 
...
To run firmware on the Raspberry Pi Pico 1 or 2/STM32U545 using the `probe-rs` extension 
...
1. Ensure `"svdFile": "./rp2350.svd"`, `"svdFile": "./rp2040.svd"` or `"svdFile": "./stm32u545.svd"` is set in `launch.json`.  
