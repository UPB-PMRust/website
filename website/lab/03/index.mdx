...
When using the *USB protocol*, however, a more stable and precise clock signal is required. The USB specification demands a tightly controlled 48 MHz clock, which is typically derived from the external crystal (HSE) through the PLL. For this reason, the HSE is necessary for reliable USB operation.

Unlike the Raspberry Pi Pico, which comes with a fixed 12 MHz crystal oscillator, the STM32 Nucleo-U545RE-Q does not have a built-in crystal frequency. Instead, the board designer selects an appropriate crystal (commonly 8 MHz, 16 MHz, or 24 MHz) to drive the HSE, depending on the application requirements.
  </TabItem>
...
1. The code above is an example for PA0 with TIM2_CH1. You need to modify the timer, channel, and pin depending on which PWM‑capable pin you want to use!

2. The duty cycle is not set through `compare_a` or `compare_b` like on RP2040. Instead, you use the set_duty_cycle() method on the channel handle. 
:::
...
<Tabs>
  <TabItem value="stm32u5" label="STM32 Nucleo-U545RE-Q" default>
    On the STM32, the ADC peripheral does not use a FIFO interrupt like the RP2040. Instead, each ADC (ADC1, ADC2, etc.) can be configured directly, and Embassy provides a driver that handles resolution, averaging, and sample time for you. All we need to do is create the ADC driver, configure it, and then bind it to the pin we want to read from.
...
loop {
    ...
    let level: u16 = adc.read(&mut adc_pin).await.unwrap();
