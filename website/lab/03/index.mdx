...
STM32 timers tick at the timer input clock $$f_{tim}$$ divided by the prescaler ($$psc+1$$). The period in ticks is the auto-reload value $$arr$$.
...
- $$f_{tim}$$ is the maximum frequency of the core/timer
- $$T$$ is the target period

#### Converting pulse width to compare value

For PWM mode, the duty cycle is set via $$ccr$$ and is relative to $$arr$$. Given the desired pulse width $$pulse_{width}$$:

$$

duty_{cycle} = \left( \frac{pulse_{width} \times arr}{T} \right)
...
// Enable the PWM channel
    ...
    const MIN_PERIOD_MS: u32 = 500;
    const MAX_PERIOD_MS: u32 = 2500;
    const PERIOD: u32 = 20000;

    let min_value = (MIN_PERIOD_MS * 1000) / PERIOD;
    let max_value = (MAX_PERIOD_MS * 1000) / PERIOD;

    // Main loop to move the servo back and forth
    loop {
        // At 50 Hz, T = 20 ms. A 2.5 ms pulse means 2.5/20 = 12.5% duty cycle.
        // Use set_duty_cycle_fraction(max_value, 1000) to set servo to ~180°.
        // Wait 1 second before moving back
        // A 0.5 ms pulse means 0.5/20 = 2.5% duty cycle.
        // Use set_duty_cycle_fraction(min_value, 1000) to set servo to ~0°.
