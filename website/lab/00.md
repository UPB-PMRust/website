...
:::tip
This lab is rather long, but it tries to be a really quick intro to Rust. We suggest going directly to the [Exercises](#exercises), solve them one by one, and read the required documentation as you go.
:::
...
The data type of a constant must be specified at declaration.
...
When the match expression runs, it compares the resulting value to the pattern for each arm, in order. If a pattern matches the value, the code associated with that pattern is executed. If this pattern does not match the value, execution continues to the next arm.
...
You may place a `?` after a call that returns a result value, and it will immediately propagate the error to the caller, otherwise continue as if the expression is the value inside the `Ok` variant
...
The data type that Rust standard library provides for storing a list of data is `Vec`. This is similar to the C++ `vector` and Java `ArrayList`.

The data type of a vector is `Vec<T>`, where `T` can be any data type.

To create a new vector, Rust provides the `vec!` macro. A longer way of writing is `Vec::new()`.
...
The best way to iterate over all the elements in a `Vec` is using a `for`.
...
3. Write a function that returns the first even number from an array slice. Make sure to handle the case where there is no even number. (**1p**) *Hint: a slice is a part of an array, `&a[first..end]`. Take a look at [`for`](#for) and [`Option`](#option-enum). Keep in mind that for gives a reference to each element in the list.*
...
## Bonus at home
1. Rewrite the function at 2., but this time implement it using the [Sieve of Eratosthenes](https://www.geeksforgeeks.org/sieve-of-eratosthenes/).
