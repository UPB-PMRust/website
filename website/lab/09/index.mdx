...
```rust
// Add a vendor-specific function (class 0xFF) and interface
...
```rust
// Task 1: Run the core USB driver
    let usb_driver = usb.run();

    // Task 2: The actual application logic
...
3. **Host-side Rust Client (2p)**: Write a Rust script using nusb that:
