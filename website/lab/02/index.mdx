...
On the `STM32U5`, the idea is the same, but the implementation uses the `external interrupt (EXTI) system`. Instead of a plain `Input`, you wrap the pin in an `ExtiInput`, which connects the GPIO to its interrupt line:
