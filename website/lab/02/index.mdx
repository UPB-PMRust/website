...
The STM32U545 control the GPIO pins through one main GPIO block per port, but inside it you can think of three roles:
1. *Electrical Configuration* - manages the physical pin on the outside of the chip. It decides the electrical behavior of the pad, such as whether it has a pull‑up or pull‑down resistor, whether it drives strongly or weakly, and whether it is push‑pull or open‑drain.
2. *Alternate function multiplexer* - connects the internal peripherals to the outside pins. Each pin can be switched to different functions. For example, a single pin can be used as:
    * A normal `GPIO` input/output
    * A `UART` transmit line
    * An `I2C` data line
    * An `SPI` clock or data line
3. *GPIO Input/Output* - this is the simple digital interface that developers use when they want to read or write a pin directly in software. It’s what you use to toggle an LED or read a button.


![STM32 Nucleo-U545RE-Q Pinout](images/stm32u5-pinout.jpeg)

:::info

The pins in pink represent the Arduino header pins.
...
| [`embassy_stm32`](https://docs.embassy.dev/embassy-stm32/git/stm32u545re/index.html) | HAL Implementation, [`embassy_rp`](https://docs.embassy.dev/embassy-rp/git/rp2040/index.html) | HAL Implementation |
...
<Tabs>
  <TabItem value="stm32u5" label="STM32 Nucleo-U545RE-Q" default>
    ```toml
# STM32U545 HAL
embassy-stm32 = { version = "0.2.0", git = "https://github.com/embassy-rs/embassy", rev = "2e7a2b6", features = ["defmt", "stm32u545re", "unstable-pac", "memory-x", "time-driver-tim4", "exti", "chrono"] }
...
embassy-time = { version = "0.4.0", git = "https://github.com/embassy-rs/embassy", rev = "2e7a2b6", features = ["defmt", "defmt-timestamp-uptime", "tick-hz-32_768"] }
...
For Raspberry Pi Pico you just have to change the feature in `embassy-rp` like this:

```toml
embassy-rp = { version = "0.3.0", git = "https://github.com/embassy-rs/embassy", rev = "2e7a2b6", features = ["defmt", "unstable-pac", "time-driver", "critical-section-impl", "rp2040", "binary-info"] }
...
right in and use them.  
  ...
  <TabItem value="rp2350" label="Raspberry Pi Pico 2/Raspberry Pi Pico" default>
```rust
#![no_main]
#![no_std]
...
// initialize PX_n (replace x with a letter(see the datasheet, e.g. PA5) and n with a number) and set its
// default value to LOW (0)
let mut pin = Output::new(peripherals.Px_n, Level::Low, Speed::Medium);
...
let pin = Input::new(peripherals.Px_n, Pull::Up);
